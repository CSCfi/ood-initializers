...
system({"LD_LIBRARY_PATH" => "/appl/opt/ood/#{ENV["SLURM_OOD_ENV"]}/lib:#{ENV["LD_LIBRARY_PATH"]}"}, "/appl/opt/ood/#{ENV["SLURM_OOD_ENV"]}/soft/ood-csc-projects", "-b", "/tmp/#{ENV["USER"]}_ood_balance.json", "-q", "/tmp/#{ENV["USER"]}_ood_quotas.json", "-r", "0.05")
