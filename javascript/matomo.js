import { getMatomoEnvironment } from "/assets/modules/locals.js";
const [MATOMO_SITE_ID, MATOMO_URL] = getMatomoEnvironment(window.location.hostname);
...
const getStrippedArray = array => array.slice(0, SENSITIVE_PATHNAME_CUTOFF_DEPTH);
...
const getStrippedPathname = pathname => {
  ...
  const strippedPathArray = containsRemoteName(pathname)
    ? getMaskedArray(pathArray)
    : getStrippedArray(pathArray);

  return arrayToPathname(strippedPathArray);
};
